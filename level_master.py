...
def play_master(screen, world_surface, width, height, clock, font, vignette_surface):
    running = True
    next_state = "menu"
    ...
    level_keys = [pygame.Rect(kx, ky, 20, 10) for kx, ky in master_keys]
    keys_collected_status = [False] * len(level_keys)

    level_coins = [pygame.Rect(cx, cy, 20, 20) for cx, cy in master_coins]
    coins_collected_status = [False] * len(level_coins)

    level_lives = [pygame.Rect(lx, ly, 20, 20) for lx, ly in master_lives]
